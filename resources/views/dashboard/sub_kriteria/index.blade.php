                    ...
                    @foreach ($data as $item)
                        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden mb-10">
                            <div class="flex justify-between items-center d p-4">
                                <div class="flex space-x-3">
                                    <div class="flex space-x-3 items-center">
                                        <h2 class="text-xl font-bold text-gray-800 dark:text-gray-200">Sub Kriteria: <span
                                                class="text-purple-600 dark:text-purple-300">{{ $item['kriteria'] }}</span>
                                        </h2>
                                    </div>
                                </div>
                                <div class="flex space-x-3">
                                    <div class="flex space-x-3 items-center">
                                        <label for="add_button" id="label_{{ $item['kriteria_id'] }}"
                                        ...
                                        @if ($subKriteria->where('kriteria_id', $item['kriteria_id'])->count() != $kategori->count())
                                            <button class="btn btn-sm btn-warning text-white normal-case"
                                                onclick="buttonMatriksDisable()">
                                                <i class="ri-pencil-fill"></i>
                                                Matriks
                                            </button>
                                        @else
                                            <a href="{{ route('perhitungan_kriteria.ubah', ['kriteria_id' => $item['kriteria_id']]) }}"
                                                class="btn btn-sm btn-warning text-white normal-case">
                                                <i class="ri-pencil-fill"></i>
                                                Matriks
                                            </a>
                                        @endif
                            ...
                            <div class="overflow-x-auto p-3">
                                <table id="{{ 'tabel_data_' . $item['kriteria_id'] }}"
                                            ...
                                            <tr class="border-b dark:border-gray-700">
                                                ...
                                                <td class="px-4 py-3">
                                                    <label for="edit_button" class="btn btn-sm btn-warning text-white"
                                                        onclick="return edit_button('{{ $subKriteria['id'] }}', '{{ $item['kriteria'] }}')">
            ...
            <div class="modal">
                <div class="modal-box">
                    <form action="{{ route('sub_kriteria.simpan') }}" method="post" enctype="multipart/form-data">
                        <h3 class="font-bold text-lg">Tambah {{ $judul }} <span class="text-purple-600"
                                id="title_add_button"></span></h3>
                        @csrf
                        ...
                        <div class="form-control w-full max-w-xs">
                            ...
                            <select name="kategori_id" class="select select-bordered w-full max-w-xs text-gray-800"
                                required>
            ...
            <div class="modal">
                <div class="modal-box" id="edit_form">
                    <form action="{{ route('sub_kriteria.perbarui') }}" method="post" enctype="multipart/form-data">
                        <h3 class="font-bold text-lg">Ubah {{ $judul }}: <span
                                class="text-purple-600 dark:text-purple-300" id="title_form"><span
...
@section('js')
    <script>
        ...
        window.delete_button = function(id, nama, kriteria) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        error: function(response) {
                            ...
                            if (response.responseJSON && response.responseJSON.message && response
                                .responseJSON.message.includes('foreign key constraint')) {
                                msg =
                                    'Data gagal dihapus karena masih digunakan pada penilaian. Silakan hapus data penilaian terkait terlebih dahulu.';
                            }
                            Swal.fire({
                                icon: 'error',
